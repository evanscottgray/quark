...
git checkout ip_policy_inside_subnet
quark-db-manage  --config-file ~/etc/neutron.conf upgrade head
python bin/print_all_subnets_ip_policies --config-file ~/etc/neutron.conf > RM7210.$region

diff master.$region RM7210.$region
